...
exports.LOGIN_COMPLETE_MESSAGE = void 0;
...
exports.LOGIN_COMPLETE_MESSAGE = { type: 'login-complete' };
 ...
 */
const storeAuthToken = authTokenStore_1.setStoredAuthToken;
/**
 ...
 */
async function broadcastLoginAndRefreshActiveTab() {
    if (typeof chrome === 'undefined')
        return;
    try {
        chrome.runtime?.sendMessage?.(exports.LOGIN_COMPLETE_MESSAGE);
    }
    catch {
 ...
 */
async function initPopup(doc = document) {
    ...
    els.form.addEventListener('submit', async (event) => {
        ...
        setLoading(els, true);
        try {
            const data = await (0, api_1.apiFetch)('/auth/login', {
                method: 'POST',
            ...
            showError(els, err instanceof Error && err.message ? err.message : 'Sign in failed.');
        }
        finally {
