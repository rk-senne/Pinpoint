...
describe('authRateLimiter', () => {
  ...
  it('allows up to 10 requests per (IP, email) pair within the 15-minute window (Req 19.1)', async () => {
    for (let i = 0; i < 10; i++) {
  ...
  it('rejects the 11th request with 429 + Retry-After + AUTH_RATE_LIMIT envelope (Req 19.1, 19.2)', async () => {
    for (let i = 0; i < 10; i++) {
      await request(app)
        .post('/auth/login')
        .send({ email: 'user@example.com', password: 'pw' });
  ...
  it('keeps separate buckets per email from the same IP (an attacker grinding many accounts is throttled per-account)', async () => {
    // Burn 10 attempts on user-a. user-b on the same IP should still
  ...
  it('shares the bucket across endpoints when (IP, email) match (an attacker can\'t reset the budget by switching endpoint)', async () => {
    // 10 hits on /auth/login then a /auth/register hit with the same
    // IP+email is the 11th request to the limiter and must be 429.
    for (let i = 0; i < 10; i++) {
      await request(app)
        .post('/auth/login')
